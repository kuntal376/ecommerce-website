...
__export(integrated_charts_scene_exports, {
  ...
  toRadians: () => import_ag_charts_core41.toRadians
});
...
function visibleTickRange(ticks, reversed, visibleRange) {
  if (visibleRange == null || visibleRange[0] === 0 && visibleRange[1] === 1)
    return;
  const vt0 = (0, import_ag_charts_core5.clamp)(0, Math.floor(visibleRange[0] * ticks.length), ticks.length);
  const vt1 = (0, import_ag_charts_core5.clamp)(0, Math.ceil(visibleRange[1] * ticks.length), ticks.length);
...
function unpackDomainMinMax(domain) {
  const min = (0, import_ag_charts_core5.readIntegratedWrappedValue)(domain.at(0));
  const max = (0, import_ag_charts_core5.readIntegratedWrappedValue)(domain.at(-1));
  return min != void 0 && max != void 0 ? [min, max] : [void 0, void 0];
}

// packages/ag-charts-community/src/scale/colorScale.ts
var convertColorStringToOklcha = (v) => {
  const color = import_ag_charts_core6.Color.fromString(v);
  const [l, c, h] = import_ag_charts_core6.Color.RGBtoOKLCH(color.r, color.g, color.b);
  return { l, c, h, a: color.a };
};
var delta = 1e-6;
var isAchromatic = (x) => x.c < delta || x.l < delta || x.l > 1 - delta;
var interpolateOklch = (x, y, d) => {
  d = (0, import_ag_charts_core6.clamp)(0, d, 1);
  let h;
  if (isAchromatic(x)) {
    h = y.h;
  ...
  return import_ag_charts_core6.Color.fromOKLCH(l, c, h, a);
};
var ColorScale = class extends AbstractScale {
  ...
  update() {
    ...
    if (domain.length < 2) {
      import_ag_charts_core6.Logger.warnOnce("`colorDomain` should have at least 2 values.");
      if (domain.length === 0) {
        domain.push(0, 1);
    ...
    for (let i = 1; i < domain.length; i++) {
      ...
      const b = domain[i];
      if (a > b) {
        import_ag_charts_core6.Logger.warnOnce("`colorDomain` values should be supplied in ascending order.");
  ...
  refresh() {
    ...
    this.update();
    if (this.invalid) {
      import_ag_charts_core6.Logger.warnOnce("Expected update to not invalidate scale");
    }
  }
};
__decorateClass([
...
var Gradient = class {
  ...
  toSvg(shapeBbox) {
    ...
    for (const { stop: offset, color } of this.stops) {
      const stop = (0, import_ag_charts_core7.createSvgElement)("stop");
...
var ConicGradient = class extends Gradient {
  ...
  createCanvasGradient(ctx, bbox, params) {
    ...
    const { angle } = this;
    const radians = (0, import_ag_charts_core8.normalizeAngle360FromDegrees)(angle + angleOffset);
  ...
  createSvgGradient(_bbox) {
    return (0, import_ag_charts_core8.createSvgElement)("linearGradient");
  }
};
...
var LinearGradient = class extends Gradient {
  ...
  createSvgGradient(bbox) {
    const { x0, y0, x1, y1 } = this.getGradientPoints(bbox);
    const gradient = (0, import_ag_charts_core9.createSvgElement)("linearGradient");
...
var RadialGradient = class extends Gradient {
  ...
  createSvgGradient(bbox) {
    ...
    const gradient = (0, import_ag_charts_core10.createSvgElement)("radialGradient");
...
var StopProperties = class extends import_ag_charts_core11.BaseProperties {
  constructor() {
    super(...arguments);
    this.color = "black";
  }
...
function getColorStops(baseFills, defaultColorStops, domain, fillMode = "continuous") {
  ...
  if (fills.length === 0) {
    ...
    import_ag_charts_core11.Logger.warnOnce(`[fills] must have the stops defined in ascending order`);
    return [];
  }
...
var Image2 = class {
  ...
  createCanvasImage(ctx, image, width, height) {
    ...
    if (renderedWidth < 1 || renderedHeight < 1) {
      import_ag_charts_core12.Logger.warnOnce("Image fill is too small to render, ignoring.");
      return null;
    }
  ...
  setImageTransform(pattern, bbox) {
    ...
    const angle = (0, import_ag_charts_core12.normalizeAngle360FromDegrees)(rotation);
  ...
  toSvg(bbox, pixelRatio) {
    ...
    const pattern = (0, import_ag_charts_core12.createSvgElement)("pattern");
    ...
    const rect = (0, import_ag_charts_core12.createSvgElement)("rect");
    ...
    const image = (0, import_ag_charts_core12.createSvgElement)("image");
...
function parseSvg(d) {
  ...
  while (i < d.length) {
    ...
    if (commandMatch == null) {
      if (!currentCommand) {
        import_ag_charts_core13.Logger.warnOnce(`Invalid SVG path, error at index ${i}: Missing command.`);
        return;
      }
...
var ExtendedPath2D = class {
  ...
  ellipse(cx, cy, rx, ry, rotation, sAngle, eAngle, counterClockwise = false) {
    ...
    let sweep = counterClockwise ? -(0, import_ag_charts_core14.normalizeAngle360)(sAngle - eAngle) : (0, import_ag_charts_core14.normalizeAngle360)(eAngle - sAngle);
    ...
    for (let ci = 0, pi = 0; ci < cn; ci++) {
      switch (commands[ci]) {
        ...
        case 1 /* Line */: {
          ...
          cx = params[pi++];
          cy = params[pi++];
          best = (0, import_ag_charts_core14.lineDistanceSquared)(x, y, x0, y0, cx, cy, best);
          break;
        }
        case 2 /* Curve */: {
          const cp0x = cx;
          ...
          cx = params[pi++];
          cy = params[pi++];
          best = (0, import_ag_charts_core14.bezier2DDistance)(cp0x, cp0y, cp1x, cp1y, cp2x, cp2y, cx, cy, x, y) ** 2;
          break;
        }
        case 3 /* ClosePath */:
          best = (0, import_ag_charts_core14.lineDistanceSquared)(x, y, cx, cy, sx, sy, best);
          break;
      }
    }
    return best;
    ...
    for (let ci = 0; ci < this.commandsLength; ci++) {
      ...
      switch (command) {
        ...
        case 2 /* Curve */: {
          ...
          const ts = (0, import_ag_charts_core14.bezier2DExtrema)(cp0x, cp0y, cp1x, cp1y, cp2x, cp2y, cp3x, cp3y);
          for (const t of ts) {
            const px = (0, import_ag_charts_core14.evaluateBezier)(cp0x, cp1x, cp2x, cp3x, t);
            const py = (0, import_ag_charts_core14.evaluateBezier)(cp0y, cp1y, cp2y, cp3y, t);
            joinPoint(px, py);
          }
          joinPoint(cp3x, cp3y);
          break;
...
var PATTERNS = {
  ...
  hearts(path, { width, height, padding }) {
    ...
    path.arc(x - r, y - r, r, (0, import_ag_charts_core15.toRadians)(130), (0, import_ag_charts_core15.toRadians)(330));
    path.arc(x + r, y - r, r, (0, import_ag_charts_core15.toRadians)(220), (0, import_ag_charts_core15.toRadians)(50));
    path.lineTo(x, y + r);
    path.closePath();
  },
  crosses(path, params) {
...
var Pattern = class {
  ...
  createCanvasPattern(ctx, pixelRatio) {
    ...
    if (width * scale < 1 || height * scale < 1) {
      import_ag_charts_core16.Logger.warnOnce("Pattern fill is too small to render, ignoring.");
      return null;
    }
  ...
  setPatternTransform(pattern, pixelRatio, tx = 0, ty = 0) {
    if (pattern == null)
      return;
    const angle = (0, import_ag_charts_core16.normalizeAngle360FromDegrees)(this.rotation);
  ...
  toSvg() {
    ...
    const pattern = (0, import_ag_charts_core16.createSvgElement)("pattern");
    pattern.setAttribute("viewBox", `0 0 ${width} ${height}`);
    pattern.setAttribute("width", String(width));
    pattern.setAttribute("height", String(height));
    pattern.setAttribute("patternUnits", "userSpaceOnUse");
    const rect = (0, import_ag_charts_core16.createSvgElement)("rect");
    rect.setAttribute("x", "0");
    rect.setAttribute("y", "0");
    rect.setAttribute("width", String(width));
    rect.setAttribute("height", String(height));
    ...
    rect.setAttribute("fill-opacity", String(backgroundFillOpacity));
    pattern.appendChild(rect);
    const path = (0, import_ag_charts_core16.createSvgElement)("path");
...
var _Shape = class _Shape extends Node {
  ...
  getGradient(fill) {
    if ((0, import_ag_charts_core17.isGradientFill)(fill))
  ...
  getPattern(fill) {
    if ((0, import_ag_charts_core17.isPatternFill)(fill))
  ...
  getImage(fill) {
    if ((0, import_ag_charts_core17.isImageFill)(fill))
  ...
  onFillChange() {
    if (typeof this.fill === "object") {
      if ((0, import_ag_charts_core17.objectsEqual)(this._cachedFill ?? {}, this.fill)) {
        return;
      }
    }
  ...
  applySvgFillAttributes(element, defs) {
    ...
    if (typeof fill === "string") {
    ...
    } else if ((0, import_ag_charts_core17.isGradientFill)(fill) && this.fillGradient) {
      defs ?? (defs = []);
      const gradient = this.fillGradient.toSvg(this.fillBBox ?? this.getBBox());
      const id = (0, import_ag_charts_core17.generateUUID)();
      gradient.setAttribute("id", id);
      defs.push(gradient);
      element.setAttribute("fill", `url(#${id})`);
    } else if ((0, import_ag_charts_core17.isPatternFill)(fill) && this.fillPattern) {
      defs ?? (defs = []);
      ...
      defs.push(pattern);
      element.setAttribute("fill", `url(#${id})`);
    } else if ((0, import_ag_charts_core17.isImageFill)(fill) && this.fillImage) {
      defs ?? (defs = []);
      const pixelRatio = this.layerManager?.canvas?.pixelRatio ?? 1;
  ...
  applySvgStrokeAttributes(element) {
    ...
    setSvgStrokeAttributes(element, { stroke: (0, import_ag_charts_core17.isString)(stroke) ? stroke : void 0, strokeOpacity, strokeWidth });
  ...
  setFillProperties(fill, fillBBox, fillParams) {
    const computedFillBBox = fillBBox == null || !(0, import_ag_charts_core17.isGradientFill)(fill) || fill.bounds == null || fill.bounds === "item" ? void 0 : fillBBox[fill.bounds];
    let hasDirectChanges = false;
...
__decorateClass([
  (0, import_ag_charts_core17.DeclaredSceneChangeDetection)()
...
__decorateClass([
  (0, import_ag_charts_core17.DeclaredSceneObjectChangeDetection)({
    equals: import_ag_charts_core17.objectsEqual,
    changeCb: _Shape.handleFillChange
  })
], _Shape.prototype, "fill", 2);
__decorateClass([
  (0, import_ag_charts_core17.SceneObjectChangeDetection)({ equals: import_ag_charts_core17.objectsEqual, changeCb: _Shape.handleStrokeChange })
], _Shape.prototype, "stroke", 2);
__decorateClass([
  (0, import_ag_charts_core17.DeclaredSceneChangeDetection)()
], _Shape.prototype, "strokeWidth", 2);
__decorateClass([
  (0, import_ag_charts_core17.SceneArrayChangeDetection)()
...
__decorateClass([
  (0, import_ag_charts_core17.DeclaredSceneChangeDetection)({ convertor: (v) => (0, import_ag_charts_core17.clamp)(0, v ?? 1, 1) })
], _Shape.prototype, "opacity", 2);
__decorateClass([
  (0, import_ag_charts_core17.SceneObjectChangeDetection)({ equals: import_ag_charts_core17.TRIPLE_EQ, checkDirtyOnAssignment: true })
], _Shape.prototype, "fillShadow", 2);
__decorateClass([
  (0, import_ag_charts_core17.DeclaredSceneObjectChangeDetection)({ equals: import_ag_charts_core17.boxesEqual, changeCb: (s) => s.onFillChange() })
], _Shape.prototype, "fillBBox", 2);
__decorateClass([
  (0, import_ag_charts_core17.DeclaredSceneObjectChangeDetection)({ equals: import_ag_charts_core17.objectsEqual, changeCb: (s) => s.onFillChange() })
...
var Matrix = class _Matrix {
  ...
  get identity() {
    const e = this.elements;
    return (0, import_ag_charts_core18.isNumberEqual)(e[0], 1) && (0, import_ag_charts_core18.isNumberEqual)(e[1], 0) && (0, import_ag_charts_core18.isNumberEqual)(e[2], 0) && (0, import_ag_charts_core18.isNumberEqual)(e[3], 1) && (0, import_ag_charts_core18.isNumberEqual)(e[4], 0) && (0, import_ag_charts_core18.isNumberEqual)(e[5], 0);
  }
  /**
  ...
  class MatrixTransformInternal extends ParentNode {
    ...
    toSVG() {
      ...
      if (matrix.identity || svg == null)
        return svg;
      const g = (0, import_ag_charts_core19.createSvgElement)("g");
...
var _Group = class _Group extends Node {
   ...
   */
  append(nodes) {
    for (const node of (0, import_ag_charts_core20.toIterable)(nodes)) {
      node.remove();
...
__decorateClass([
  (0, import_ag_charts_core3.SceneChangeDetection)({ convertor: (v) => (0, import_ag_charts_core20.clamp)(0, v, 1) })
...
var StatsAccumulator = class {
  ...
  startPeriodicLogging() {
    if (!import_ag_charts_core21.Debug.check("scene:stats" /* SCENE_STATS */, "scene:stats:verbose" /* SCENE_STATS_VERBOSE */)) {
      return;
    }
  ...
  logAccumulatedStats() {
    ...
    import_ag_charts_core21.Logger.log(`\u{1F4CA} Stats (${timeSinceLastLog.toFixed(0)}s, ${count} renders): ${parts.join(" ")}`);
    this.stats.clear();
    this.lastLogTime = Date.now();
  }
  destroy() {
...
function debugStats(layersManager, debugSplitTimes, ctx, renderCtxStats, extraDebugStats = {}, seriesRect = BBox.zero) {
  ...
  const detailedStats = import_ag_charts_core21.Debug.check("scene:stats:verbose" /* SCENE_STATS_VERBOSE */);
  const memUsage = detailedStats ? memoryUsage() : null;
  const metrics = detailedStats ? import_ag_charts_core21.DebugMetrics.flush() : {};
  ...
  const stats = [
    ...
    memUsage
  ].filter(import_ag_charts_core21.isString);
  const measurer = new import_ag_charts_core21.TextMeasurer(ctx);
  ...
  try {
    ...
    import_ag_charts_core21.Logger.warnOnce("Error during debug stats rendering", e);
  } finally {
    ctx.restore();
  }
}
function prepareSceneNodeHighlight(ctx) {
  const config = (0, import_ag_charts_core21.toArray)((0, import_ag_charts_core21.getWindow)("agChartsSceneDebug"));
  const result = [];
...
function debugSceneNodeHighlight(ctx, debugNodes) {
  ...
  try {
    for (const [name, node] of Object.entries(debugNodes)) {
      ...
      if (!bbox) {
        import_ag_charts_core21.Logger.log(`Scene.render() - no bbox for debugged node [${name}].`);
        continue;
      }
      ctx.globalAlpha = 0.8;
      ctx.strokeStyle = "red";
    ...
    import_ag_charts_core21.Logger.warnOnce("Error during debug rendering", e);
  } finally {
    ctx.restore();
  }
}
...
function buildTree(node, mode) {
  if (!import_ag_charts_core21.Debug.check(true, "scene" /* SCENE */)) {
    return {};
  }
  let order = 0;
  return {
...
Path.className = "Path";
__decorateClass([
  (0, import_ag_charts_core22.SceneChangeDetection)()
...
function cornerEdges(leadingEdge, trailingEdge, leadingInset, trailingInset, cornerRadius) {
  ...
  } else if ((0, import_ag_charts_core23.isNumberEqual)(leading0, 0)) {
    leading0 = 0;
  }
  ...
  } else if ((0, import_ag_charts_core23.isNumberEqual)(trailing1, 0)) {
    trailing1 = 0;
  }
...
Rect.className = "Rect";
__decorateClass([
  (0, import_ag_charts_core23.DeclaredSceneChangeDetection)()
], Rect.prototype, "x", 2);
__decorateClass([
  (0, import_ag_charts_core23.DeclaredSceneChangeDetection)()
], Rect.prototype, "y", 2);
...
__decorateClass([
  (0, import_ag_charts_core23.DeclaredSceneChangeDetection)({ equals: import_ag_charts_core23.boxesEqual })
...
var _Text = class _Text extends Shape {
  ...
  onTextChange() {
    this.richText?.clear();
    this.textMap?.clear();
    if ((0, import_ag_charts_core24.isArray)(this.text)) {
      this.lines = [];
      this.richText ?? (this.richText = new Group());
      this.richText.setScene(this.scene);
      this.richText.append(
        this.text.flatMap((s) => (0, import_ag_charts_core24.toTextString)(s.text).split(import_ag_charts_core24.LineSplitter)).filter(Boolean).map(() => new _Text({ trimText: false }))
      );
    } else {
      const lines = (0, import_ag_charts_core24.toTextString)(this.text).split(import_ag_charts_core24.LineSplitter);
      this.lines = this.trimText ? lines.map((line) => line.trim()) : lines;
    }
  }
  get font() {
    this.fontCache ?? (this.fontCache = (0, import_ag_charts_core24.toFontString)(this));
    return this.fontCache;
  }
  static measureBBox(text, x, y, options) {
    if ((0, import_ag_charts_core24.isArray)(text)) {
      const { font, lineHeight, textAlign, textBaseline } = options;
      const { width, height, lineMetrics } = (0, import_ag_charts_core24.measureTextSegments)(text, font);
      ...
      return _Text.computeBBox((0, import_ag_charts_core24.toTextString)(text).split(import_ag_charts_core24.LineSplitter), x, y, options);
    }
  }
  static computeBBox(lines, x, y, opts) {
    const { font, lineHeight, textAlign, textBaseline } = opts;
    const { width, height, lineMetrics } = (0, import_ag_charts_core24.cachedTextMeasurer)(font).measureLines(lines);
    const totalHeight = lineHeight ? lineHeight * lineMetrics.length : height;
    const offsetTop = _Text.calcTopOffset(totalHeight, lineMetrics[0], textBaseline);
    const offsetLeft = _Text.calcLeftOffset(width, textAlign);
    return new BBox(x - offsetLeft, y - offsetTop, width, totalHeight);
  ...
  getPlainText() {
    return (0, import_ag_charts_core24.toPlainText)(this.text);
  }
  isPointInPath(x, y) {
  ...
  generateTextMap() {
    if (!(0, import_ag_charts_core24.isArray)(this.text) || this.textMap?.size)
      return;
    this.textMap ?? (this.textMap = /* @__PURE__ */ new Map());
    let offsetY = 0;
    const textNodes = this.richText.children();
    for (const { width, height, ascent, segments } of (0, import_ag_charts_core24.measureTextSegments)(this.text, this).lineMetrics) {
      let offsetX = 0;
  ...
  render(renderCtx) {
    ...
    if ((0, import_ag_charts_core24.isArray)(this.text) && this.richText) {
      this.generateTextMap();
      const richTextBBox = this.richText.getBBox();
      const { width, height, lineMetrics } = (0, import_ag_charts_core24.measureTextSegments)(this.text, this);
      let translateX = 0;
      ...
      switch (this.textBaseline) {
        ...
        case "middle":
          translateY -= lineMetrics.length === 1 ? lineMetrics[0].ascent + lineMetrics[0].segments.reduce(
            (offsetY, segment) => Math.min(offsetY, (0, import_ag_charts_core24.cachedTextMeasurer)(segment).baselineDistance("middle")),
            0
          ) : height / 2;
          break;
        case "bottom":
  ...
  renderLines(renderCallback) {
    ...
    if (!Number.isFinite(x) || !Number.isFinite(y))
      return;
    const measurer = (0, import_ag_charts_core24.cachedTextMeasurer)(this);
  ...
  toSVG() {
    ...
    if (text == null)
      return;
    const element = (0, import_ag_charts_core24.createSvgElement)("text");
    if ((0, import_ag_charts_core24.isArray)(text)) {
      for (const segment of text) {
        const segmentElement = (0, import_ag_charts_core24.createSvgElement)("tspan");
        ...
        segmentElement.textContent = (0, import_ag_charts_core24.toTextString)(segment.text);
      ...
      element.textContent = (0, import_ag_charts_core24.toTextString)(text);
    }
  ...
  hasRenderableText() {
    ...
    return (0, import_ag_charts_core24.isArray)(text) ? true : (0, import_ag_charts_core24.toTextString)(text) !== "";
  }
};
_Text.className = "Text";
_Text.debug = import_ag_charts_core24.Debug.create(true, "scene:text" /* SCENE_TEXT */);
...
__decorateClass([
  (0, import_ag_charts_core24.SceneRefChangeDetection)({
...
// packages/ag-charts-community/src/chart/caption.ts
var Caption = class extends import_ag_charts_core25.BaseProperties {
  constructor() {
    super(...arguments);
    this.id = (0, import_ag_charts_core25.createId)(this);
    ...
    this.fontSize = import_ag_charts_core25.FONT_SIZE.SMALLER;
    this.fontFamily = "sans-serif";
  ...
  computeTextWrap(containerWidth, containerHeight) {
    ...
    if ((0, import_ag_charts_core25.isArray)(text)) {
      wrappedText = (0, import_ag_charts_core25.wrapTextSegments)(text, options);
      this.truncated = wrappedText.some(import_ag_charts_core25.isSegmentTruncated);
    } else {
      wrappedText = (0, import_ag_charts_core25.wrapText)((0, import_ag_charts_core25.toTextString)(text), options);
      this.truncated = (0, import_ag_charts_core25.isTextTruncated)(wrappedText);
    }
  ...
  updateA11yText(moduleCtx, where) {
    ...
    const textContent = (0, import_ag_charts_core25.toPlainText)(this.text);
  ...
  handleMouseMove(moduleCtx, event) {
    if (event != null && this.enabled && this.truncated) {
      ...
      moduleCtx.tooltipManager.updateTooltip(this.id, { canvasX, canvasY, showArrow: false }, [
        { type: "structured", title: (0, import_ag_charts_core25.toPlainText)(this.text) }
      ]);
    }
  }
...
__decorateClass([
  ...
  (0, import_ag_charts_core25.ProxyPropertyOnWrite)("node", "visible")
], Caption.prototype, "enabled", 2);
__decorateClass([
  import_ag_charts_core25.Property,
  (0, import_ag_charts_core25.ProxyPropertyOnWrite)("node")
...
__decorateClass([
  import_ag_charts_core25.Property,
  (0, import_ag_charts_core25.ProxyPropertyOnWrite)("node", "fill")
...
var MARKER_SHAPES = {
  ...
  heart({ path, x, y, size }) {
    const r = size / 4;
    y = y + r / 2;
    path.arc(x - r, y - r, r, (0, import_ag_charts_core26.toRadians)(130), (0, import_ag_charts_core26.toRadians)(330));
    path.arc(x + r, y - r, r, (0, import_ag_charts_core26.toRadians)(220), (0, import_ag_charts_core26.toRadians)(50));
    path.lineTo(x, y + r);
    path.closePath();
  },
  pin({ path, x, y, size: s }) {
...
__decorateClass([
  (0, import_ag_charts_core27.DeclaredSceneChangeDetection)()
], InternalMarker.prototype, "x", 2);
__decorateClass([
  (0, import_ag_charts_core27.DeclaredSceneChangeDetection)()
], InternalMarker.prototype, "y", 2);
__decorateClass([
  (0, import_ag_charts_core27.DeclaredSceneChangeDetection)({ convertor: Math.abs })
...
var CategoryScale = class _CategoryScale extends BandScale {
  ...
  ticks(params, domain = this.domain, visibleRange) {
    ...
    step = (0, import_ag_charts_core29.previousPowerOf2)(step);
    if (step <= 1) {
      return filterVisibleTicks(domain, false, visibleRange);
    }
    tickCount = Math.trunc(bands.length / step);
    const span = step * tickCount;
    const inset = (0, import_ag_charts_core29.previousPowerOf2)(Math.trunc((bands.length - span) / 2));
    const vt0 = (0, import_ag_charts_core29.clamp)(0, Math.floor((visibleRange?.[0] ?? 0) * bands.length), bands.length);
    const vt1 = (0, import_ag_charts_core29.clamp)(0, Math.ceil((visibleRange?.[1] ?? 1) * bands.length), bands.length);
  ...
  findIndex(value) {
    ...
    if (!indexInitialized) {
      ...
      for (let i = 0; i < domain.length; i++) {
        index.set((0, import_ag_charts_core29.dateToNumber)(domain[i]), i);
      }
      this.indexInitialized = true;
    }
    return index.get((0, import_ag_charts_core29.dateToNumber)(value));
  }
};
function deduplicateCategories(d) {
  ...
  for (const value of d) {
    const key = (0, import_ag_charts_core29.dateToNumber)(value);
...
var _ContinuousScale = class _ContinuousScale extends AbstractScale {
  ...
  convert(value, options) {
    ...
    if (clamp8) {
      const [start, stop] = (0, import_ag_charts_core30.findMinMax)([d0, d1]);
      if (x < start) {
        return range2[0];
      } else if (x > stop) {
        return range2[1];
...
var LinearScale = class _LinearScale extends ContinuousScale {
  ...
  static getTickStep(start, stop, ticks) {
    ...
    return interval ?? (0, import_ag_charts_core31.tickStep)(start, stop, tickCount, minTickCount, maxTickCount);
  }
  toDomain(d) {
    return d;
  }
  ticks({ interval, tickCount = ContinuousScale.defaultTickCount, minTickCount, maxTickCount }, domain = this.domain, visibleRange) {
    ...
    if (interval) {
      ...
      if (!(0, import_ag_charts_core31.isDenseInterval)((d1 - d0) / step, this.getPixelRange())) {
        return (0, import_ag_charts_core31.range)(d0, d1, step, visibleRange);
      }
    }
    return (0, import_ag_charts_core31.createTicks)(d0, d1, tickCount, minTickCount, maxTickCount, visibleRange);
  }
  niceDomain(ticks, domain = this.domain) {
    ...
    if (tickCount === 1) {
      [start, stop] = (0, import_ag_charts_core31.niceTicksDomain)(start, stop);
    } else if (tickCount > 1) {
...
__decorateClass([
  (0, import_ag_charts_core32.ObserveChanges)((target) => target.onEnabledChange())
...
var LayersManager = class {
  constructor(canvas) {
    this.canvas = canvas;
    this.debug = import_ag_charts_core34.Debug.create(true, "scene");
...
var Scene = class extends import_ag_charts_core35.EventEmitter {
  constructor(canvasOptions) {
    super();
    this.debug = import_ag_charts_core35.Debug.create(true, "scene" /* SCENE */);
    this.id = (0, import_ag_charts_core35.createId)(this);
    this.imageLoader = new ImageLoader();
    this.root = null;
    this.pendingSize = null;
    this.isDirty = false;
    this.cleanup = new import_ag_charts_core35.CleanupRegistry();
  ...
  updateDebugFlags() {
    import_ag_charts_core35.Debug.inDevelopmentMode(() => Node._debugEnabled = true);
  }
  clearCanvas() {
    this.canvas.clear();
  }
  ...
  render(opts) {
    ...
    const statsEnabled = import_ag_charts_core35.Debug.check("scene:stats" /* SCENE_STATS */, "scene:stats:verbose" /* SCENE_STATS_VERBOSE */);
    if (statsEnabled) {
    ...
    if (import_ag_charts_core35.Debug.check("scene:stats:verbose" /* SCENE_STATS_VERBOSE */)) {
      renderCtx.stats = {
        layersRendered: 0,
        layersSkipped: 0,
        nodesRendered: 0,
    ...
    if (root && import_ag_charts_core35.Debug.check("scene:dirtyTree" /* SCENE_DIRTY_TREE */)) {
      const { dirtyTree, paths } = buildDirtyTree(root);
      import_ag_charts_core35.Debug.create("scene:dirtyTree" /* SCENE_DIRTY_TREE */)("Scene.render() - dirtyTree", { dirtyTree, paths });
    }
...
var Arc = class extends Path {
  ...
  get fullPie() {
    return (0, import_ag_charts_core36.isNumberEqual)((0, import_ag_charts_core36.normalizeAngle360)(this.startAngle), (0, import_ag_charts_core36.normalizeAngle360)(this.endAngle));
  }
  updatePath() {
...
Arc.className = "Arc";
__decorateClass([
  (0, import_ag_charts_core36.SceneChangeDetection)()
...
var Line = class extends Shape {
  ...
  distanceSquared(px, py) {
    ...
    return (0, import_ag_charts_core37.lineDistanceSquared)(px, py, x1, y1, x2, y2, Infinity);
  }
  render(renderCtx) {
...
var RadialColumnShape = class extends Path {
  ...
  getRotation() {
    ...
    const midAngle = (0, import_ag_charts_core38.angleBetween)(startAngle, endAngle);
    return (0, import_ag_charts_core38.normalizeAngle360)(startAngle + midAngle / 2 + Math.PI / 2);
  }
  ...
  updateBeveledPath() {
    ...
    const bottom = -innerRadius;
    const rotation = this.getRotation();
    const isTouchingInner = (0, import_ag_charts_core38.isNumberEqual)(innerRadius, axisInnerRadius);
    const isTouchingOuter = (0, import_ag_charts_core38.isNumberEqual)(outerRadius, axisOuterRadius);
...
__decorateClass([
  (0, import_ag_charts_core38.SceneChangeDetection)()
...
function getRadialColumnWidth(startAngle, endAngle, axisOuterRadius, columnWidthRatio, maxColumnWidthRatio) {
  const rotation = (0, import_ag_charts_core38.angleBetween)(startAngle, endAngle);
...
function sectorBox({ startAngle, endAngle, innerRadius, outerRadius }) {
  ...
  if ((0, import_ag_charts_core39.isBetweenAngles)(0, startAngle, endAngle)) {
    addPoint(outerRadius, 0);
  }
  if ((0, import_ag_charts_core39.isBetweenAngles)(Math.PI * 0.5, startAngle, endAngle)) {
    addPoint(0, outerRadius);
  }
  if ((0, import_ag_charts_core39.isBetweenAngles)(Math.PI, startAngle, endAngle)) {
    addPoint(-outerRadius, 0);
  }
  if ((0, import_ag_charts_core39.isBetweenAngles)(Math.PI * 1.5, startAngle, endAngle)) {
    addPoint(0, -outerRadius);
  }
  return new BBox(x0, y0, x1 - x0, y1 - y0);
}
function isPointInSector(x, y, sector) {
  ...
  const startAngle = (0, import_ag_charts_core39.normalizeAngle180)(sector.startAngle);
  const endAngle = (0, import_ag_charts_core39.normalizeAngle180)(sector.endAngle);
  const angle = Math.atan2(y, x);
...
function clockwiseAngle(angle, relativeToStartAngle) {
  if ((0, import_ag_charts_core39.angleBetween)(angle, relativeToStartAngle) < delta2) {
    return relativeToStartAngle;
  } else {
    return (0, import_ag_charts_core39.normalizeAngle360)(angle - relativeToStartAngle) + relativeToStartAngle;
  }
}
function clockwiseAngles(startAngle, endAngle, relativeToStartAngle = 0) {
  const fullPie = Math.abs(endAngle - startAngle) >= 2 * Math.PI;
  const sweepAngle = fullPie ? 2 * Math.PI : (0, import_ag_charts_core39.normalizeAngle360)(endAngle - startAngle);
...
__decorateClass([
  (0, import_ag_charts_core40.SceneChangeDetection)()
...
__decorateClass([
  (0, import_ag_charts_core40.SceneObjectChangeDetection)({ equals: (lhs, rhs) => lhs.equals(rhs) })
