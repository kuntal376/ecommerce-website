...
const MenFootwear = ({ onDataChange }) => {

    const [details, setDetails] = useState({
        type: '',        // e.g. Sneakers, Loafers, Boots
        material: '',    // e.g. Leather, Suede, Canvas, Mesh
        closureType: '', // e.g. Lace-up, Slip-on, Velcro (Added to fill 4th line)
  ...
  return (
    <div className='flex items-center gap-3'>
      ...
      <div className='w-[60%] bg-[#f1f1f1] px-4 rounded-md py-3'>
        ...
        {/* Line 1: Type */}
        <div className='mt-5'>
            <label htmlFor="type" className="block mb-2.5 text-sm font-medium text-heading">Footwear Type:</label>
            <input type="text" name="type" onChange={handleChange} id="type" className="bg-[#e3e3e3] text-heading text-sm rounded-md focus:ring-[#e3e3e3] focus:border-[#e3e3e3] w-full px-3 py-2.5 shadow-xs placeholder:text-body" placeholder="e.g. Sneakers, Loafers, Boots"/>
        </div>

        {/* Line 2: Material */}
        <div className='mt-5'>
            <label htmlFor="material" className="block mb-2.5 text-sm font-medium text-heading">Material:</label>
            <input type="text" name="material" onChange={handleChange} id="material" className="bg-[#e3e3e3] text-heading text-sm rounded-md focus:ring-[#e3e3e3] focus:border-[#e3e3e3] w-full px-3 py-2.5 shadow-xs placeholder:text-body" placeholder="e.g. Genuine Leather, Canvas, Mesh"/>
        </div>

        {/* Line 3: Closure Type (Added for layout balance) */}
        <div className='mt-5'>
            <label htmlFor="closureType" className="block mb-2.5 text-sm font-medium text-heading">Closure Type:</label>
            <input type="text" name="closureType" onChange={handleChange} id="closureType" className="bg-[#e3e3e3] text-heading text-sm rounded-md focus:ring-[#e3e3e3] focus:border-[#e3e3e3] w-full px-3 py-2.5 shadow-xs placeholder:text-body" placeholder="e.g. Lace-Up, Slip-On, Velcro"/>
        </div>

        {/* Line 4: Occasion */}
        <div className='mt-5'>
            <label htmlFor="occasion" className="block mb-2.5 text-sm font-medium text-heading">Occasion:</label>
            <input type="text" name="occasion" onChange={handleChange} id="occasion" className="bg-[#e3e3e3] text-heading text-sm rounded-md focus:ring-[#e3e3e3] focus:border-[#e3e3e3] w-full px-3 py-2.5 shadow-xs placeholder:text-body" placeholder="e.g. Casual, Formal, Running"/>
        </div>
      </div>
