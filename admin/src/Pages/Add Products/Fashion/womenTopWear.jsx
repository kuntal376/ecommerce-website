...
const WomenTopWear = ({ onDataChange }) => {

    const [details, setDetails] = useState({
        dressType: '', // e.g. Top, Blouse, Kurti, T-Shirt
        fabric: '',    // e.g. Cotton, Silk, Chiffon
        sleeveLength: '', // e.g. Sleeveless, 3/4 Sleeve, Full Sleeve
        fit: '',       // e.g. Regular Fit, Slim Fit, Oversized
        occasion: '',  // e.g. Casual, Party, Office Wear
        //market info
        sku:'',
        stockLevel:'',
        taxClass:'',
  ...
  return (
    <div className='flex items-center gap-3'>
      ...
      <div className='w-[60%] bg-[#f1f1f1] px-4 rounded-md py-3'>
        ...
        {/* Line 1: Dress Type & Fabric (Side by Side) */}
        <div className='mt-5 flex items-center gap-5'>
            <div className='w-[50%]'>
                <label htmlFor="dressType" className="block mb-2.5 text-sm font-medium text-heading">Dress Type:</label>
                <input type="text" name="dressType" onChange={handleChange} id="dressType" className="bg-[#e3e3e3] text-heading text-sm rounded-md focus:ring-[#e3e3e3] focus:border-[#e3e3e3] w-full px-3 py-2.5 shadow-xs placeholder:text-body" placeholder="e.g. Blouse, Kurti, Tank Top"/>
            </div>
            <div className='w-[50%]'>
                <label htmlFor="fabric" className="block mb-2.5 text-sm font-medium text-heading">Fabric:</label>
                <input type="text" name="fabric" onChange={handleChange} id="fabric" className="bg-[#e3e3e3] text-heading text-sm rounded-md focus:ring-[#e3e3e3] focus:border-[#e3e3e3] w-full px-3 py-2.5 shadow-xs placeholder:text-body" placeholder="e.g. Cotton, Silk, Rayon"/>
            </div>
        </div>

        {/* Line 2: Sleeve Length (Full Width) */}
        <div className='mt-5'>
            <label htmlFor="sleeveLength" className="block mb-2.5 text-sm font-medium text-heading">Sleeve Length:</label>
            <input type="text" name="sleeveLength" onChange={handleChange} id="sleeveLength" className="bg-[#e3e3e3] text-heading text-sm rounded-md focus:ring-[#e3e3e3] focus:border-[#e3e3e3] w-full px-3 py-2.5 shadow-xs placeholder:text-body" placeholder="e.g. Sleeveless, 3/4th Sleeve, Long Sleeve"/>
        </div>

        {/* Line 3: Fit (Full Width) */}
        <div className='mt-5'>
            <label htmlFor="fit" className="block mb-2.5 text-sm font-medium text-heading">Fit:</label>
            <input type="text" name="fit" onChange={handleChange} id="fit" className="bg-[#e3e3e3] text-heading text-sm rounded-md focus:ring-[#e3e3e3] focus:border-[#e3e3e3] w-full px-3 py-2.5 shadow-xs placeholder:text-body" placeholder="e.g. Regular, Slim, Oversized"/>
        </div>

        {/* Line 4: Occasion (Full Width) */}
        <div className='mt-5'>
            <label htmlFor="occasion" className="block mb-2.5 text-sm font-medium text-heading">Occasion:</label>
            <input type="text" name="occasion" onChange={handleChange} id="occasion" className="bg-[#e3e3e3] text-heading text-sm rounded-md focus:ring-[#e3e3e3] focus:border-[#e3e3e3] w-full px-3 py-2.5 shadow-xs placeholder:text-body" placeholder="e.g. Casual, Party, Formal, Ethnic"/>
        </div>
      </div>
