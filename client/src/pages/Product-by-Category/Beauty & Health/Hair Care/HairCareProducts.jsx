...
import HairCareProductsData from "./HairCareProductsData";
...
// MOCK DATA
const mockProducts = HairCareProductsData;
...
const HairCareProducts = ({ filters, apiData }) => {
  ...
  let filteredProducts = productsToDisplay.filter((product) => {
    ...
    const productType = product.specs?.productType;

    const brandMatch = filters.brand.length === 0 || filters.brand.includes(brand);
    const productTypeMatch = filters.productType.length === 0 || filters.productType.includes(productType);
  ...
  if (filteredProducts.length === 0) {
      return <div className="text-center p-10 text-gray-500">No Hair Care products found matching your filters.</div>
  }
          ...
          {filteredProducts.map((product) => {
            ...
            <div key={product.id || product._id} className="border rounded-md p-4 bg-white hover:shadow-lg transition">
              ...
              <div className="text-sm mt-3 grid grid-cols-[100px_1fr] gap-y-1">
                ...
                <span>{product.specs?.productType}</span>
