...
import ToyandGameProductsData from "./ToyandGameProductsData";

// MOCK DATA
const mockProducts = ToyandGameProductsData
...
const ToyandGameProducts = ({ filters, apiData }) => {
  ...
  let filteredProducts = productsToDisplay.filter((product) => {
    ...
    const categoryTypeMatch = filters.categoryType.length === 0 || filters.categoryType.includes(product.specs?.categoryType);
  ...
  if (filteredProducts.length === 0) {
      return <div className="text-center p-10 text-gray-500">No Toy & Game found matching your filters.</div>
  }
          ...
          {filteredProducts.map((product) => {
            ...
            <div key={product.id || product._id} className="border rounded-md p-4 bg-white hover:shadow-lg transition">
              ...
              <div className="text-sm mt-3 grid grid-cols-[100px_1fr] gap-y-2">
                ...
                {product.specs?.categoryType && (
                  <>
                    <span className="font-medium text-gray-500">Product Type:</span>
                    <span>{product.specs?.categoryType}</span>
                  </>
                )}
...
export default ToyandGameProducts;