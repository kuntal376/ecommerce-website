...
import TabletProductsData from './TabletProductsData' 

const mockProducts = TabletProductsData
...
const TabletProducts = ({ filters, apiData }) => {
  ...
  if (filteredProducts.length === 0) {
      return <div className="text-center p-10 text-gray-500">No tablets found matching your filters.</div>
  }
      ...
      {filteredProducts.map((product) => {
        ...
        <div key={product.id || product._id} className="border rounded-md p-4 bg-white hover:shadow-lg transition">
          ...
          <div className="text-sm mt-3 grid grid-cols-[90px_1fr] gap-y-1">
            <span className="font-medium text-gray-500">RAM:</span><span>{product.specs?.ram}</span>
