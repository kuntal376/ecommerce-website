...
import BagsLuggagesProductsData from "./BagsLuggagesProductsData";

// MOCK DATA
const mockProducts = BagsLuggagesProductsData;
...
const BagsLuggagesProducts = ({ filters, apiData }) => {
  ...
  let filteredProducts = productsToDisplay.filter((product) => {
    ...
    const materialMatch = filters.material.length === 0 || filters.material.includes(product.specs?.material);
    const brandMatch = filters.brand.length === 0 || filters.brand.includes(product.brand);
    const capacityMatch = filters.capacity.length === 0 || filters.capacity.includes(product.specs?.capacity);
    const finalPrice = getFinalPrice(product);
    const priceMatch = maxPrice !== null ? finalPrice <= maxPrice : true;

    return (
      typeMatch && materialMatch && brandMatch && priceMatch && capacityMatch
    );
  })
  // ---------- SORT ----------
      ...
      {filteredProducts.map((product) => {
        ...
        <div key={product.id || product._id} className="border rounded-md p-4 bg-white hover:shadow-lg transition">
          ...
          <div className="text-sm mt-3 grid grid-cols-[90px_1fr] gap-y-1">
            <span className="font-medium text-gray-500">Type:</span><span>{product.specs?.type}</span>
            <span className="font-medium text-gray-500">Material:</span><span>{product.specs?.material}</span>
            <span className="font-medium text-gray-500">Occasion:</span><span>{product.specs?.occasion}</span>
            <span className="font-medium text-gray-500">Capacity:</span><span>{product.specs?.capacity}</span>
          </div>
        </div>
      )})}
    </div>
