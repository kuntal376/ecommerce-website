...
const MenTopWear = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            try {
                // Fetch specifically for 'Men's Top-Wear' subcategory
                const data = await getProductsByCategory("Men's Top-Wear");
                if (data && Array.isArray(data)) {
                    setApiProducts(data);
                }
            } catch (error) {
                console.error("Failed to load Men's Top Wear from API.");
            } finally {
                setIsLoading(false);
            }
        };
            ...
            <section className="Fashion-section">
                <div className="Fashion-bars flex flex-col md:flex-row gap-4 p-4">
                    ...
                    <div className="w-full md:w-1/4 lg:w-1/5">
                        <MenTopWearFilter filters={filters} setFilters={setFilters} subCategory= "Men's Top-Wear"/>
                    </div>
                    ...
                    <div className="w-full md:w-3/4 lg:w-4/5">
                        {isLoading ? (
                            ...
                            <MenTopWearProducts apiData={apiProducts} filters={filters} />
                        )}
                    </div>
