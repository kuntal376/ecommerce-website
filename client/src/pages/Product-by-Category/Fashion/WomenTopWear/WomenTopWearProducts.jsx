...
import WomenTopWearProductsData from "./WomenTopWearProductsData";

// MOCK DATA
const mockProducts = WomenTopWearProductsData
...
const WomenTopWearProducts = ({ filters, apiData }) => {
  ...
  let filteredProducts = productsToDisplay.filter((product) => {
    ...
    // Support potential variations in property names from API
      ...
      {filteredProducts.map((product) => {
        ...
        <div key={product.id || product._id} className="border rounded-md p-4 bg-white hover:shadow-lg transition">
          ...
          <div className="text-sm mt-3 grid grid-cols-[90px_1fr] gap-y-1">
            ...
            <span className="font-medium text-gray-500">Sleeve:</span><span>{product.specs?.sleeveLength}</span>
