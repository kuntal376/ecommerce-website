...
import HomeComfortProductsData from "./HomeComfortProductsData";
...
// MOCK DATA
const mockProducts = HomeComfortProductsData;
...
const HomeComfortProducts = ({ filters, apiData }) => {
  ...
  let filteredProducts = productsToDisplay.filter((product) => {
    ...
    const subType = product.specs?.subType;
    ...
    const subTypeMatch = filters.subType.length === 0 || filters.subType.includes(subType);
  ...
  if (filteredProducts.length === 0) {
      return <div className="text-center p-10 text-gray-500">No home comfort products found matching your filters.</div>
  }
          ...
          {filteredProducts.map((product) => {
            ...
            <div key={product.id || product._id} className="border rounded-md p-4 bg-white hover:shadow-lg transition">
              ...
              <div className="text-sm mt-3 grid grid-cols-[100px_1fr] gap-y-1">
                ...
                <span>{product.specs?.applianceType}</span>

                {product.specs?.subType && (
                  <>
                    <span className="font-medium text-gray-500">Type:</span>
                    <span>{product.specs?.subType}</span>
                  </>
                )}
                ...
                {product.specs?.features && (
                    <>
                        ...
                        <span className="truncate block" title={product.specs?.features}>
                            {product.specs?.features}
                        </span>
                    </>
                )}
...
export default HomeComfortProducts;