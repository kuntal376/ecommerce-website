...
const HomeComfortsFilter = ({ filters, setFilters, subCategory }) => {
  ...
  useEffect(() => {
    const fetchFilters = async () => {
        ...
        if (!data) return;
        // Map the API data to the filters state keys
        const mappedFilters = {
            brand: data.brand || [],
  ...
  return (
    <aside className="w-full bg-white border rounded-md p-4 shadow-sm h-min">
      ...
      <div className="mb-4 border-b pb-4">
        ...
        {open.price && (
          <div className="mt-3 space-y-2 text-sm text-gray-600">
            {[1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000].map((p) => (
...
export default HomeComfortsFilter;