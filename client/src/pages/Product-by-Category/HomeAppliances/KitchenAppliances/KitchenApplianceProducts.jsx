...
// MOCK DATA
const mockProducts = KitchenApplianceProductsData;
...
const KitchenApplianceProducts = ({ filters, apiData }) => {
  ...
  let filteredProducts = productsToDisplay.filter((product) => {
    ...
    const powerValue = getPowerValue(power);

    const brandMatch = filters.brand.length === 0 || filters.brand.includes(brand);
    const capacityMatch = filters.capacity.length === 0 || filters.capacity.includes(capacity);
  ...
  if (filteredProducts.length === 0) {
      return <div className="text-center p-10 text-gray-500">No kitchen appliances found matching your filters.</div>
  }
        ...
        <div className="h-min grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 flex-1">
          {filteredProducts.map((product) => {
            ...
            <div key={product.id || product._id} className="border rounded-md p-4 bg-white hover:shadow-lg transition">
              ...
              <h3 className="text-sm font-medium mt-4 text-center sm:text-left">{product.brand} {product.name}</h3>
              ...
              <div className="text-sm mt-3 grid grid-cols-[100px_1fr] gap-y-1">
                ...
                {product.specs?.features && (
                  <>
                    ...
                    <span className="truncate block" title={product.specs?.features}>{product.specs?.features}</span>
                  </>
                )}
