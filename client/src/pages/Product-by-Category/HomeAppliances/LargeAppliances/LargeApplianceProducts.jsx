...
// MOCK DATA
const mockProducts = LargeApplianceProductsData;
...
const LargeApplianceProducts = ({ filters, apiData }) => {
  ...
  if (filteredProducts.length === 0) {
      return <div className="text-center p-10 text-gray-500">No large appliances found matching your filters.</div>
  }
          ...
          {filteredProducts.map((product) => {
            ...
            <div key={product.id || product._id} className="border rounded-md p-4 bg-white hover:shadow-lg transition">
              ...
              <div className="text-sm mt-3 grid grid-cols-[100px_1fr] gap-y-1">
                ...
                {product.specs?.capacityValue && product.specs?.capacityUnit && (
                  <>
                    <span className="font-medium text-gray-500">Capacity:</span>
                    <span>{product.specs?.capacityValue} {product.specs?.capacityUnit}</span>
                  </>
                )}
