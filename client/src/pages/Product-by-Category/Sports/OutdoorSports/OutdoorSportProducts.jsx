...
import OutdoorSportProductsData from "./OutdoorSportProductsData";
...
// MOCK DATA
const mockProducts = OutdoorSportProductsData;
...
const OutdoorSportProducts = ({ filters, apiData }) => {
  ...
  let filteredProducts = productsToDisplay.filter((product) => {
    ...
    // Note: Ensure API provides 'sport' field
    const sportMatch = filters.sport.length === 0 || filters.sport.includes(product.specs?.sport);
  ...
  if (filteredProducts.length === 0) {
      return <div className="text-center p-10 text-gray-500">No outdoor sports products found matching your filters.</div>
  }
              ...
              <div className="text-sm mt-3 grid grid-cols-[90px_1fr] gap-y-2">
                ...
                {product.specs?.sport && (
                  <>
                    ...
                    <span className="text-justify text-gray-600 line-clamp-3">{product.specs?.sport}</span>
                  </>
                )}
...
export default OutdoorSportProducts;